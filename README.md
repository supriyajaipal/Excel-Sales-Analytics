# Excel-Sales-Analytics
Business-focused Excel analytics project including Customer Performance, Market vs Target, and P & L analysis across fiscal years, markets, and months.

## Project Objective

### Sales Reports

1. Created [Customer Performance Report] (https://github.com/supriyajaipal/Excel-Sales-Analytics/blob/main/Customer%20Performance%20Report.pdf)

2. Compared [Market Performance vs Sales Targets] (https://github.com/supriyajaipal/Excel-Sales-Analytics/blob/main/Market%20Performance%20vs%20Target%20Report.pdf)

### Finance Reports

1. Created P & L reports by [Fiscal Years] (https://github.com/supriyajaipal/Excel-Sales-Analytics/blob/main/P%20%26%20L%20Statement%20by%20Fiscal%20Year.pdf) and [Months] (https://github.com/supriyajaipal/Excel-Sales-Analytics/blob/main/P%20%26%20L%20Statement%20by%20Months.pdf)

2. Created P & L reports by [Markets] (https://github.com/supriyajaipal/Excel-Sales-Analytics/blob/main/P%20%26%20L%20Statement%20by%20Market.pdf)

## Reports Included

- Customer Performance Report
- Market Performance vs Target Report
- P&L by Fiscal Years
- P&L by Markets
- P&L by Months

## Tools Used

- Microsoft Excel
- Power Query
- Power Pivot
- DAX

## Technical Skills

- [x] ETL methodology (Extract, Transform, Load)
- [x] Power Query for data transformation
- [x] Generated date tables
- [x] Derived fiscal months and quarters
- [x] Built data model relationships using Power Pivot
- [x] Integrated supplementary datasets
- [x] Created calculated columns using DAX

## Business & Soft Skills

- [x] Understanding Sales & Finance reporting
- [x] User-centric report design
- [x] Report optimization and performance tuning
- [x] Structured report planning approach

## Key Insights

- Identified customer-wise sales performance trends across multiple fiscal years.
- Compared market performance against predefined sales targets.
- Analyzed P&L performance across fiscal years, markets, and months.
- Evaluated Gross Margin (GM%) and profitability patterns.
- Generated business-focused reports to support decision-making.
